...
void EV_HLDM_GunshotDecalTrace( pmtrace_t *pTrace, float *forward, char *decalName )
{
	int iRand;
	physent_t *pe;
	vec3_t vecSpot;
	...
	// pull out a little bit so particles won't drown in BSP
	vecSpot = pTrace->endpos + forward;

	gEngfuncs.pEfxAPI->R_RunParticleEffect( vecSpot, vec3_origin, 0, 12 );
...
void EV_HLDM_DecalGunshot( pmtrace_t *pTrace, float *forward, int iBulletType )
{
	physent_t *pe;
	...
	if ( pe && pe->solid == SOLID_BSP )
	{
		...
		default:
			// smoke and decal
			EV_HLDM_GunshotDecalTrace( pTrace, forward, EV_HLDM_DamageDecal( pe ) );
			break;
		}
	}
}
...
//	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/glock.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong( 0, 3 ) );
