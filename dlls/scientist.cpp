...
void CScientist :: TalkInit()
{
	...
	m_szGrp[TLK_PLDIE1] =	"!SC_FEAR7";
	m_szGrp[TLK_PLDIE2] =	"!SC_FEAR9";
...
Schedule_t *CScientist :: GetSchedule ( void )
{
	...
	case MONSTERSTATE_COMBAT:
		if ( HasConditions( bits_COND_NEW_ENEMY ) )
		{
			...
			{
				if ( m_hEnemy != NULL && m_flNextMonsterAnnouncement <= gpGlobals->time )
				{
					CBaseMonster *pMonster = m_hEnemy->MyMonsterPointer( );

					if ( pMonster )
					{
						if ( FClassnameIs( pMonster->pev, "monster_headcrab" ) )
							PlaySentence( "!SC_MONST0", 3, VOL_NORM, ATTN_NORM ); // point out head craps
						else if ( pMonster->MainGib() != MAINGIB_HUMAN && RANDOM_LONG(0,3) == 0 ) // don't brag about other species too often
							PlaySentence( "!SC_MONST1", 3, VOL_NORM, ATTN_NORM ); // regular monster

						Talk( 3 );
						m_flNextMonsterAnnouncement = gpGlobals->time + 30.0;
					}
				}
			}
