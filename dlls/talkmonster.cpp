...
TYPEDESCRIPTION	CTalkMonster::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CTalkMonster, m_iFollowAttempts, FIELD_INTEGER ),
};
...
void CTalkMonster :: StartTask( Task_t *pTask )
{
	...
	case TASK_MOVE_TO_TARGET_RANGE:
		m_iFollowAttempts = 0;	// successful path, forget all the follow attempts
	...
	case TASK_CANT_FOLLOW:
		if ( m_flNextCantFollowSentence <= gpGlobals->time )
		{	
			m_flNextCantFollowSentence = gpGlobals->time + 2.5;
			m_iFollowAttempts++;
		}

		if ( m_iFollowAttempts >= 3)
		{
			StopFollowing( FALSE );
...
Schedule_t* CTalkMonster :: GetScheduleOfType ( int Type )
{
	...
	case SCHED_ALERT_FACE:
		{
			// UNDONE: react to player sound while they're unseen
			BOOL fSuspicious = FALSE;
			CSound *pSound;
			pSound = PBestSound();

			ALERT ( at_aiconsole, "%s check\n", STRING(pev->classname) );

			// is this sound even worth commenting on?
			if ( pSound && pSound->m_iType & (bits_SOUND_DANGER | bits_SOUND_COMBAT) )
				fSuspicious = TRUE;

			if (!FBitSet(m_bitsSaid, bit_saidHeard) && fSuspicious)
			{
				ALERT ( at_aiconsole, "hear\n" );

				CBaseEntity *pFriend = NULL;
				int i;

				// delay friends from commenting...
				for ( i = 0; i < TLK_CFRIENDS; i++ )
				{
					while (pFriend = EnumFriends( pFriend, i, TRUE ))
					{
						...
						if ( pMonster )
						{
							if ( !(pMonster->pev->flags & FL_CLIENT) )
							{
								if ( !FBitSet(pMonster->m_bitsSaid, bit_saidHeard) )
								{
									SetBits(pMonster->m_bitsSaid, bit_saidHeard);
									pMonster->m_flNextHearForget = gpGlobals->time + RANDOM_FLOAT(5.0, 10.0);
								}
							}
						}
					}
				...
				PlaySentence( m_szGrp[TLK_HEAR], RANDOM_FLOAT(2.8, 3.2), VOL_NORM, ATTN_IDLE );
				
				// delay self even further
				SetBits(m_bitsSaid, bit_saidHeard);
				m_flNextHearForget = gpGlobals->time + RANDOM_FLOAT(30.0, 60.0);
				
				return slIdleStand;
			}
			else return CBaseMonster::GetScheduleOfType( Type ); // no, just turn towards it
		}
...
void CTalkMonster :: PrescheduleThink ( void )
{
	...
	if ( gpGlobals->time > m_flNextHearForget && FBitSet(m_bitsSaid, bit_saidHeard) )
	{
		ALERT ( at_aiconsole, "%s forget hear\n", STRING(pev->classname) );
		ClearBits(m_bitsSaid, bit_saidHeard);
	}
}
	...
	if ( IsFollowing() )
	{
		if ( !(m_afMemory & bits_MEMORY_PROVOKED) )
		{
			if ( m_hTargetEnt->IsAlive() )
				...
				PlaySentence( m_szGrp[TLK_PLDIE1 + RANDOM_LONG(0,1)], RANDOM_FLOAT(2.8, 3.2), VOL_NORM, ATTN_IDLE );
	...
	if ( pCaller != NULL && pCaller->IsPlayer() )
	{
		...
		else if ( CanFollow() )
		{
			...
			else
			{
				...
				if ( m_iTriggerCondition == AITRIGGER_USED_BY_PLAYER )
				{
