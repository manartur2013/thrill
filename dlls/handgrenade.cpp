...
void CHandGrenade::WeaponIdle( void )
{
	...
	if ( m_flStartThrow )
	{
		...
		// this code is only useful when a certain amount of x-axis puchangle applied, which rarely happens
		if ( angThrow.x == -90 )
...
//			flVel = 500;
		...
		if ( angThrow.x <= 5 && angThrow.x >= -15 )
			SendWeaponAnim( HANDGRENADE_THROW2 );
		else
			SendWeaponAnim( HANDGRENADE_THROW3 );

//		the values here aren't realistic, the velocity rarely gets above 500
/*		if ( flVel < 500 )
		{
			SendWeaponAnim( HANDGRENADE_THROW1 );
		}
