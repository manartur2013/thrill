...
{
//	ALERT(at_console, "Flying Monster: CheckLocalMove\n");
...
{ 
//	ALERT(at_console, "Flying Monster: GetStoppedActivity\n");
...
{ 
//	ALERT(at_console, "Flying Monster: Stop\n");
...
{
//	ALERT(at_console, "Flying Monster: MoveExecute\n");
	if ( pev->movetype == MOVETYPE_FLY )
	{
		...
		else
		{
			...
			m_stopTime = gpGlobals->time;
			m_vecTravel = g_vecZero;
			ALERT(at_console, "Flying Monster: LOCALMOVE_INVALID\n");
		}		
	}
	else
...
//=========================================================
// FPathBlocked - returns TRUE if there is an obstacle ahead
//=========================================================
BOOL CFlyingMonster :: FPathBlocked( )
{
	TraceResult		tr;
	...
	UTIL_TraceLine(pev->origin, pev->origin + gpGlobals->v_forward * 192, ignore_monsters, ENT(pev), &tr);
	if (tr.flFraction != 1.0)
	{
		m_flLastBlockedTime = gpGlobals->time;
		fBlocked = TRUE;
	...
	// extra wide checks
	UTIL_TraceLine(pev->origin + gpGlobals->v_right * 12, pev->origin + gpGlobals->v_right * 12 + gpGlobals->v_forward * 192, ignore_monsters, ENT(pev), &tr);
	if (tr.flFraction != 1.0)
	{
		m_flLastBlockedTime = gpGlobals->time;
		fBlocked = TRUE;
...
//	if ( fBlocked ) ALERT(at_console, "Flying Monster: Path is blocked!\n");
