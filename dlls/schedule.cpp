	...
	case TASK_STRAFE_TO_TARGET:
		{
			...
			if ( (m_hTargetEnt->pev->origin - pev->origin).Length() < 1 )
			...
			else
				{
					...
					{
						// strafe right
						ALERT( at_console, "SCRIPTED: Strafing right!\n" );
						newActivity = ACT_STRAFE_RIGHT;
					}
					else
					{
						// strafe left
						ALERT( at_console, "SCRIPTED: Strafing left!\n" );
						newActivity = ACT_STRAFE_LEFT;
					}
	...
	case TASK_FACE_SCRIPT:
		{
			...
			//	ALERT( at_console, "SCRIPTED: MakeIdealYaw!\n" );
			//	MakeIdealYaw( m_hTargetEnt->pev->origin );
