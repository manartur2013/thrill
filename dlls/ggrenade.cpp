...
void CGrenade::Explode( TraceResult *pTrace, int bitsDamageType )
{
	...
	{
		vecSpot = pTrace->vecEndPos + (pTrace->vecPlaneNormal * (pev->dmg - 24) * 0.6);
	}

	PLAYBACK_EVENT_FULL( NULL, NULL, g_sExplode, 0.0, (float*)&vecSpot, (float*)&g_vecZero, pev->dmg, 0.0, 0, 0, 0, 0);
...
class CTestGrenade : public CBaseDelay
{
public:
	void Spawn( void );
	void Use ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value );
};
LINK_ENTITY_TO_CLASS( test_grenade, CTestGrenade );

void CTestGrenade::Spawn( void )
{
	pev->movetype = MOVETYPE_NONE;
	pev->solid = SOLID_NOT;
}

void CTestGrenade :: Use ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	CGrenade::ShootContact( pev, pev->origin, pev->origin + (gpGlobals->v_up * -8) );
}