...
void CEgon::Precache( void )
{
	...
	m_usEgonSwitch = PRECACHE_EVENT ( 1, "events/egon_switch.sc" );
}
...
void CEgon::SecondaryAttack( void )
{
	...
	int prevMode = m_fireMode;

	if ( m_fireMode == FIRE_NARROW )
	{
		m_fireMode = FIRE_WIDE;
		m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + EGON_SWITCH_WIDE_TIME;
	}
	else
	{
		m_fireMode = FIRE_NARROW;
		m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + EGON_SWITCH_NARROW_TIME;
	}
	...
	PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usEgonSwitch, 0, (float *)&g_vecZero, (float *)&g_vecZero, 0.0, 0.0, prevMode, 0, 0, 0 );
}
...
//	ALERT ( at_console, "Egon: WeaponIdle\n" );
