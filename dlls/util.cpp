...
void UTIL_ShowMessage( const char *pString, CBaseEntity *pEntity )
{
	...
	EMIT_SOUND(ENT(pEntity->edict()), CHAN_STATIC, "Common/TALK.WAV", 1.0, ATTN_NORM);
	CLIENT_PRINTF( ENT( pEntity->edict() ), print_center, pString );
...
void UTIL_DrawBeam( Vector vecStart, Vector vecEnd, int r, int g, int b )
{
...
void UTIL_GunshotDecalTrace( TraceResult *pTrace, int decalNumber )
{
	...
	// UNDONE: make this an event
	UTIL_ParticleEffect( pTrace->vecEndPos, Vector(0, 0, 0), 0, 12 );
}


void UTIL_Sparks( const Vector &position )
{
	PLAYBACK_EVENT_FULL( NULL, NULL, g_sSparks, 0.0, (float*)&position, (float*)&g_vecZero, 0.0, 0.0, 0, 0, 0, 0);
}
...
{
	if ( CVAR_GET_FLOAT("sv_ricochet") == 0 )
		return;
