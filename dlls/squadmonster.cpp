	...
	else 
	{
		while ((pEntity = UTIL_FindEntityInSphere( pEntity, pev->origin, searchRadius )) != NULL)
		{
			...
			if ( pRecruit && pRecruit != this && pRecruit->IsAlive() && !pRecruit->m_pCine )
			{
				...
				// hgrunts and hsarges have different classnames, so we need another check
				if ( FClassnameIs( pev, "monster_human_grunt" ) || FClassnameIs( pev, "monster_human_assault" ) )
				{
					if ( !pRecruit->InSquad() && pRecruit->Classify() == iMyClass &&
				   ( FClassnameIs( pRecruit->pev, "monster_human_grunt" ) || FClassnameIs( pRecruit->pev, "monster_human_assault" ) ) &&
				    FStringNull( pRecruit->pev->netname ) )
					canRecruit = TRUE;
				}
				else if ( !pRecruit->InSquad() && pRecruit->Classify() == iMyClass &&
...
BOOL CSquadMonster :: NoFriendlyFire( void )
{
	...
	vecLeftSide = pev->origin - ( gpGlobals->v_right * ( pev->size.x * 1.0 ) );	// 1.5
	vecRightSide = pev->origin + ( gpGlobals->v_right * ( pev->size.x * 1.0 ) );
	...
	for (int i = 0; i < MAX_SQUAD_MEMBERS; i++)
	{
		...
		if (pMember && pMember != this)
		{
				 ...
				 rightPlane.PointInFront ( pMember->pev->origin) )
			{
				...
				//	ALERT ( at_console, "Friendly in the way\n" );
				if ( CVAR_GET_FLOAT("sv_drawroute") )
					UTIL_ParticleEffect( EyePosition(), pev->origin + gpGlobals->v_up * 16, 79, 4 );
