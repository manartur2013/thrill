...
#define	SF_GIBSHOOTER_STICKYGIB		2
#define	SF_GIBSHOOTER_MIXED			4
...
void CBeam::BeamDamage( TraceResult *ptr )
{
	...
	if ( ptr->flFraction != 1.0 && ptr->pHit != NULL )
	{
		...
		if ( pHit )
		{
			if ( FBitSet(pHit->pev->flags, FL_MONSTER) && FBitSet(pev->spawnflags, SF_BEAM_NO_MONSTERS) )
			{
				ALERT(at_console,"env_laser skips %s\n", STRING(pHit->pev->classname));
				return;
			}
			ClearMultiDamage();
...
CGib *CGibShooter :: CreateGib ( void )
{
	...
	if ( pev->spawnflags & SF_GIBSHOOTER_STICKYGIB )
	{
	...
	else if ( pev->spawnflags & SF_GIBSHOOTER_MIXED )
	{
		if ( RANDOM_LONG(0,1) )
		{	
			pGib->Spawn( "models/stickygib.mdl" );
	...
	else pGib->Spawn( "models/hgibs.mdl" );
	pGib->m_bloodColor = BLOOD_COLOR_RED;
...
// UNDONE: Implement save/restore properly
...
void CItemSoda::CanTouch ( CBaseEntity *pOther )
{
	...
	// spoit sound here
	EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, "ambience/steamburst1.wav", 1, ATTN_NORM, 0, 200 );
