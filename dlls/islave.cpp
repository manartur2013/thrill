...
enum
{
	SCHED_REVIVE = LAST_COMMON_SCHEDULE + 1,
};
...
enum
{
	TASK_REVIVE = LAST_COMMON_TASK + 1,
};
...
#define		ISLAVE_DEAD			-1
#define		ISLAVE_ALIVE		0
#define		ISLAVE_REVIVING		1

// UNDONE: Add proper resurrection feature with use of cut animations
...
public:
	...
	int SpecialGib( void ) { return SPECIALGIB_ISLAVE; }
...
TYPEDESCRIPTION	CISlave::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CISlave, m_fCollarUsed, FIELD_BOOLEAN ),
	DEFINE_FIELD( CISlave, m_iReviveState, FIELD_INTEGER ),
	DEFINE_FIELD( CISlave, m_flNextRevive, FIELD_TIME ),
};
...
{
	ClearBeams( );
	m_fRevivingOther = FALSE;
	m_iReviveState = ISLAVE_DEAD;
...
void CISlave :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		case ISLAVE_AE_ZAP_POWERUP:
		{
			...
			if (m_hDead != NULL)
			{
				CISlave *pSlave = (CISlave *)Instance(m_hDead->edict());

				WackBeam( -1, m_hDead );
				WackBeam( 1, m_hDead );

				if ( pSlave->m_iReviveState == ISLAVE_DEAD )
				{
					this->m_fRevivingOther = TRUE;
					pSlave->TaskComplete();
					pSlave->SetThink ( &CBaseMonster::CallMonsterThink );
					pSlave->pev->nextthink += RANDOM_FLOAT(0.1, 0.4);
					pSlave->m_iReviveState = ISLAVE_REVIVING;
				}
			}
			else
			{
		...
		case ISLAVE_AE_ZAP_SHOOT:
		{
			...
			//	UTIL_TraceHull( vecDest, vecDest, dont_ignore_monsters, human_hull, m_hDead->edict(), &trace );
				UTIL_TraceLine( vecDest, vecDest, dont_ignore_monsters, m_hDead->edict(), &trace );
				...
				if ( !trace.fStartSolid )
				{
					...
					m_flNextRevive = gpGlobals->time + RANDOM_FLOAT(10,15);

					break;
				}
				else ALERT( at_console, "CIslave: Revive zap trace failure!\n");
			}
			ClearMultiDamage();
...
BOOL CISlave :: CheckRangeAttack2 ( float flDot, float flDist )
{
	...
	while ((pEntity = UTIL_FindEntityByClassname( pEntity, "monster_alien_slave" )) != NULL)
	{
		...
		{
			if (pEntity->pev->deadflag == DEAD_DEAD && pEntity->pev->renderamt == 255)
			{
				float d = (pev->origin - pEntity->pev->origin).Length();
				if (d < flDist)
				{
...
void CISlave :: RunTask ( Task_t *pTask )
{
	if ( pTask->iTask == TASK_REVIVE )
	{
		if ( m_iReviveState < ISLAVE_REVIVING )
		{
			ALERT( at_console, "CIslave: Stopped reviving!\n");
			m_Activity = ACT_RESET;
			TaskFail();
		}
		else if ( m_fSequenceFinished )
		{
			ALERT( at_console, "CIslave: Revived!\n");
			Revive();
			TaskComplete();
		}
	}
	else CSquadMonster :: RunTask ( pTask );
}
...
void CISlave :: Spawn()
{
	...
	m_flDistDieForward	= 72;
	m_flDistDieBackward = 64;
...
int CISlave :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	...
	if ( m_fRevivingOther && m_hDead != NULL )
	{
		ClearBeams();
		CISlave *pSlave = (CISlave*)Instance(m_hDead->edict());
		m_fRevivingOther = FALSE;
...
//		m_IdealActivity = GetSmallFlinchActivity();
		m_flNextRevive = gpGlobals->time + RANDOM_FLOAT(5,10);
		pSlave->m_iReviveState = ISLAVE_DEAD;
	}
...
Task_t	tlSlaveRevive[] =
{
	{ TASK_SET_FAIL_SCHEDULE,	(float)SCHED_DIE	},
	{ TASK_REVIVE,				(float)0			},
};

Schedule_t	slSlaveRevive[] =
{
	{ 
		tlSlaveRevive,
		ARRAYSIZE ( tlSlaveRevive ), 
		0, 
		0,
		"Slave Revive"
	},
...
// dead enemy
	...
	case MONSTERSTATE_DEAD:
		if ( m_iReviveState == ISLAVE_REVIVING )
			return GetScheduleOfType( SCHED_REVIVE );
		else
...
void CISlave :: Revive( )
{
	m_iReviveState = ISLAVE_ALIVE;
	pev->deadflag = DEAD_NO;
	Forget( bits_MEMORY_KILLED );
	UTIL_SetSize(pev, VEC_HUMAN_HULL_MIN, VEC_HUMAN_HULL_MAX);
	MonsterInit();
	pev->health = pev->max_health = gSkillData.slaveHealth;
