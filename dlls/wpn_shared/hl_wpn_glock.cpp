...
void CGlock::GlockFire( float flSpread , float flCycleTime, BOOL fUseAutoAim )
{
	...
	Vector vecDir;
	vecDir = m_pPlayer->FireBulletsPlayer( 1, vecSrc, vecAiming, Vector( flSpread, flSpread, flSpread ), 8192, BULLET_PLAYER_9MM, TRACER_NEVER, 0, m_pPlayer->pev, m_pPlayer->random_seed );
...
#define		PLASMA_BEAM_SPEED	700

class CPlasmaBeam : public CBaseAnimating
{
public:
	void Spawn( void );
...
LINK_ENTITY_TO_CLASS( plasmabeam, CPlasmaBeam );

void CPlasmaBeam :: Spawn( void )
{
...
void CPlasmaBeam::Shoot( entvars_t *pevOwner, Vector vecStart, Vector vecVelocity )
{
	CPlasmaBeam *pLaser = GetClassPtr( (CPlasmaBeam *)NULL );
	pLaser->Spawn();
...
void CPlasmaBeam :: Touch ( CBaseEntity *pOther )
{
	...
	SetThink ( &CPlasmaBeam::SUB_Remove );
	pev->nextthink = gpGlobals->time;
}

class CPlasmaPistol : public CGlock
{
	void PrimaryAttack( void );
	void SecondaryAttack( void );
};

void CPlasmaPistol :: PrimaryAttack( void )
{
	
}

void CPlasmaPistol :: SecondaryAttack( void )
{
