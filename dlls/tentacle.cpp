...
public:
	...
	//	pev->absmax = pev->origin + Vector(400, 400, 850);
		pev->absmin = pev->origin + Vector(-224, -224, 0);
		pev->absmax = pev->origin + Vector(224, 224, 850);
	}
...
void CTentacle :: Spawn( )
{
	...
	if ( FBitSet( pev->spawnflags, 1024 ) )
...
// UNDONE: Add a rangeattack with radius damage
...
void CTentacleMaw :: Spawn( )
{
	...
	SET_MODEL(ENT(pev), "models/rosyanka.mdl");
	UTIL_SetSize(pev, Vector(-48, -56, 0), Vector(48, 56, 72));

	pev->solid			= SOLID_SLIDEBOX;
	pev->movetype		= MOVETYPE_FLY;
	pev->effects		= 0;
	pev->health			= 500;	// 75 -> 300 -> 1000
	pev->yaw_speed		= 0;
	pev->sequence		= 0;
...
//	PRECACHE_MODEL("models/pryanik.mdl");
	PRECACHE_MODEL("models/rosyanka.mdl");
...
int CTentacleMaw :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
