...
void CHornet :: Precache()
{
	...
	g_sHornetTrail = PRECACHE_EVENT(1, "events/hornet_trail.sc" );
...
{
	PLAYBACK_EVENT_FULL( FEV_GLOBAL, edict(), g_sHornetTrail, 0.0, (float*)&pev->origin, (float*)&g_vecZero, 0.0, 0.0, entindex(), 0, 0, 0);
}
...
void CHornet::DieTouch ( CBaseEntity *pOther )
{
	...
	{// do the damage
		...
		pOther->TakeDamage( pev, VARS( pev->owner ), pev->dmg, DMG_SLASH );
	}
