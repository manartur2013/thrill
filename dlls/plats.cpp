...
void CFuncPlat :: Blocked( CBaseEntity *pOther )
{
	...
	pOther->TakeDamage(pev, pev, flDmg, DMG_CRUSH);
...
void CFuncTrain :: HurtTouch ( CBaseEntity *pOther )
{
	float fldmg;
	...
	if ( fldmg < 0 )
		pOther->TakeHealth( -fldmg, DMG_SLASH );
	else
		pOther->TakeDamage( pev, pev, fldmg, DMG_SLASH );
...
// a little c3a1 hack - we turn train into a moveable trigger_hurt 
	if ( pev->health != 0 )
	{
		pev->solid = SOLID_TRIGGER;
		SetTouch ( &CFuncTrain::HurtTouch );
	}
}
...
{
	if (FStrEq(pkvd->szKeyName, "trainsize"))
	{
		m_size = atof(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "wheels"))
	{
...
void CFuncTrackTrain :: Spawn( void )
{
	...
	// quick hack to deal with train bounding boxes hitting stukas and turrets on c2a2
	switch(m_size)
	{
		case 0:
		default:
			...
			break;
		case 1:
			UTIL_SetSize( pev, Vector(-72, -52, -16), Vector(72, 52, 16) );
			break;
		case 2:
			UTIL_SetSize( pev, Vector(-72, -52, -16), Vector(72, 52, 0) );
			break;
	}
	UTIL_SetOrigin( pev, pev->origin );
